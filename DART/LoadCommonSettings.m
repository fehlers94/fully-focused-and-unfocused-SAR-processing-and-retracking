...
switch lower(deblank(result))
    ...
    case {'tud260502'}
        home_dir = '/home/fehelers/PhD Delft/';
        %PathHOME = fullfile(home_dir,  'Projects','IG_waves', 'Results');
        PathHOME = fullfile(home_dir,  'Projects','hydrology river', 'Results');
        PathDATA = fullfile(home_dir,  'Projects','hydrology river', 'Data');
        %PathDATA = fullfile(home_dir,  'Projects','FFSAR4ROFI', 'Data');
        PathDsof = fullfile('home','fehelers','PhD_Delft','DART_TUD', 'DART');
        PathRSLT = fullfile(home_dir,  'Projects','hydrology river', 'Results');
        Path_FIG = fullfile(home_dir,  'Projects','hydrology river', 'Results');
        
    otherwise % Everything else
        home_dir = getenv('HOME');
        PathHOME = fullfile(home_dir,  'Projects','hydrology_garonne', 'Software');
        PathDATA = fullfile(home_dir,  'Projects','hydrology_garonne', 'Data');
        PathDsof = fullfile(home_dir,  'Projects','hydrology_garonne', 'Software');
        PathRSLT = fullfile(home_dir,  'Projects','hydrology_garonne', 'Results');
        Pth_FIG  = fullfile(home_dir,  'Projects','hydrology_garonne', 'Results');
end
