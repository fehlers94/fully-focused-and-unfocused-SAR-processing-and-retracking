classdef FFSAR_Processor < handle
    properties
        FName, DOM, proc_sets
        ...
        Nl, r_res_zp, res_a, W, t, tp_vec, T, Ts, BRI %preliminary data
...
%       t_pulse, t_select, t_select_corr,
    ...
    methods      
        function obj = FFSAR_Processor(FName,DOM,FFSAR_processing_settings,focus_height_interpolator)
            LoadCommonSettings
        ...
        function read_crop_data(obj)
            ...
            if ~obj.is_cs
                ...
                if ~isempty(IDX)
                    ...
                    obj.ind_first_rel_dom_burst=NaN;
                    obj.ind_last_rel_dom_burst=NaN;
                end
                clear lat_nc lon_nc time_nc
            end
            
            if ~isempty(IDX)
                obj.nodata_flag = false;
                %Read data
                switch obj.mission
                    case 'CS'
                ...
                obj.nodata_flag = true;
                warning('L1a file contains no data in domain of interest')
            end
        end
        ...
        function setup_preliminaries(obj)
            ...
            obj.res_a=(((L^2+H^2)^0.5+obj.DDAcf.lambda0/4)^2-H^2)^0.5-L;
            Re_local=rsphere('euler',lat(1),lon(1),lat(end),lon(end),[obj.DDAcf.RefEll.SemimajorAxis obj.DDAcf.RefEll.Eccentricity]);
            obj.Nl=median(obj.CS1a.GEO.V.V(:))/(obj.res_a*((Re_local+median(obj.CS1a.GEO.H(:)))/Re_local)); % number of looks per seconds [1/s]
...
%                 obj.CS1a.GEO.Antenna_Bench_Roll(ma),...
        ...
        function init_L1b_struct(obj)
            ...
            if obj.proc_sets.split_aperture
                obj.CS1b.SAR.dataIQ_T_0 = zeros(obj.DDAcf.Np*obj.DDAcf.os_ZP, numel(obj.h_l));
                obj.CS1b.SAR.dataIQ_0_T = zeros(obj.DDAcf.Np*obj.DDAcf.os_ZP, numel(obj.h_l));
            end
            if obj.proc_sets.output_pseudo_delay_doppler_processing
                ...
                obj.CS1b.SAR.data_pseudoDD_T_0 = zeros(obj.DDAcf.Np*obj.DDAcf.os_ZP, numel(obj.h_l));
                obj.CS1b.SAR.data_pseudoDD_0_T = zeros(obj.DDAcf.Np*obj.DDAcf.os_ZP, numel(obj.h_l));
        ...
        function setup_proc(obj)
            obj.read_crop_data()
            if obj.nodata_flag
                warning('skipping all following steps: ffsar processing could not be initialized without data')
            else
                obj.mask_data()
...
%             parfor (ii=1:numel(look_loc_inds))%proc_sets.n_cores_parfor)
                ...
                estimated_time_left = (now - time_start)/ii * (numel(look_loc_inds) - ii);
                fprintf('processing nadir look location i=%d...(%d/%d), estimated time left: %s\n',i,ii,numel(look_loc_inds), datestr(estimated_time_left,'HH:MM:SS'));

                % selection of bursts (radius of obj.T second around zenith)
                t_select = t_burst(q);
                t_select_corr = t_burst_corr(q);
                
                if (numel(t_select)*obj.BRI) > 0.9*obj.T
                    if is_s6
                    ...
                    if obj.DDAcf.ApplyWindow
                        if strcmp(obj.DDAcf.WindowMode,'burst')                     
                            wav_select=wav_select.*repmat(W',size(wav_select,1)/DDAcf.Nb,1);
                        elseif strcmp(obj.DDAcf.WindowMode,'aperture')
                            if strcmpi(obj.DDAcf.Window,'hamming')
                                Wr = hamming(size(wav_select,1),'periodic');
                                wav_select=wav_select.*Wr;
                            elseif strcmpi(obj.DDAcf.Window,'hanning')
                                Wr = hanning(size(wav_select,1));
                                wav_select=wav_select.*Wr;
                            else
                                error('Window not recognized!');
                            end
                    ...
                    %% debug: apply additional range window if option is active
                    if obj.DDAcf.ApplyWindowRange
                        if strcmpi(obj.DDAcf.Window,'hamming')
                            wav_select = wav_select.*hamming(size(wav_select,2),'periodic')';
                        elseif strcmpi(obj.DDAcf.Window,'hanning')
                            wav_select = wav_select.*hanning(size(wav_select,2))';
                        else
                            error('Window not recognized!');
                        end
                    end
                    ...
                    if is_s6 && ~contains(FName, 'GPP')
                        %phase_slope = -567.8585; % fitted value over the transponder, will depend on USO frequency and typical tracker jump magnitude
                        phase_slope = -567.8585*1.002; % alternatively 2*pi*2/lambda, where lambda is c/fc
                    ...
                    % splitting the aperture to form two distinct FF-SAR waveforms with
                    % only half the resolution:
                    if proc_sets.split_aperture
                        split_ind = round(size(t_pulse,1)/2);
                    ...
                    if proc_sets.output_pseudo_delay_doppler_processing
                        ...
                        obj.CS1b.SAR.data_pseudoDD_0_T(:,i+j) = flip((sum(abs(DDdata(:,:,zD_idx:end)).^2,3))',2);
                        obj.CS1b.SAR.data_pseudoDD_T_0(:,i+j) = flip((sum(abs(DDdata(:,:,1:zD_idx-1)).^2,3))',2);
