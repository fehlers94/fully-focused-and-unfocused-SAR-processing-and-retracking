function DATA = ALL_LRM_L1b_to_L2(SAT,YY,MM,FNamesPreFix,RunID,DOM,Retracker,SetRetr)

%ALL_LRM_L1b_to_L2 produces L2 data from LRM L1b data.

% YYYY = 2010:2019;
% sprintf('qsub-eval -l CS_%i%02i.log "mlab -eval \\"ALL_LRM_L1b_to_L2(''CS'',''%i'',''%02i'',[],''ROFI'',[51 55;2 10],''SAMOSA2'',{''UseDEM2CompInitRP'',''true'';''UseLSmskInWfC'',''true''})\\""\nsleep 1s\n\n',repmat([reshape(repmat(YYYY,12,1),numel(YYYY)*12,1),reshape(repmat(1:12,numel(YYYY),1)',numel(YYYY)*12,1)],1,2)')
...
defval('RunID','TEST_SCM')                                                        %Label that identifies experiment
...
defval('Retracker','ICE1')                                                        %Retracker to be used
defval('SetRetr',{})
...
switch SAT
    case {'CS','CryoSat'}
        PathL1b = fullfile(PathDATA,'RadAlt','CryoSat','SIR_LRM_L1');             %Path to data
        FNExt   = 'DBL';
    otherwise
        error('SAT: %s not recognized',SAT)
end
PathOUT = fullfile(PathRSLT,sprintf('%s_%s_%s_%s',RunID,DARTutils.GenerateStrDOM(DOM),SAT,Retracker),'L2');  %Path to output
if ~isfolder(PathOUT); mkdir(PathOUT); end

%% Load DEM used to apply slope corrections and/or to identify the peak that has to be retracked
if ApplySlopeCorrection || UseDEM2CompInitRP
...
for i=1:numel(FILES)
    ...
    try
        ...
        fprintf('%s\n',exception.message)
        DATA = LRM_L1b_to_L2(SAT,FILES(i).name,DOM,Retracker,SetRetr,DEM,MSK);
...
for i=1:numel(names)
    if ~ismember(names{i},{'SWH','WFc','nu'})
