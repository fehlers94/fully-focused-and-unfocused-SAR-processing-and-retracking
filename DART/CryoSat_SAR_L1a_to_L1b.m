function [CS1b,CS1a] = CryoSat_SAR_L1a_to_L1b(FName,DOM,DEM)
...
defval('FName','2016/03/CS_OFFL_SIR1SAR_FR_20160302T183427_20160302T183612_C001') %*.DBL file that contains level 1a data
...
%Set paths
PathL1a = fullfile(PathDATA,'RadAlt','CryoSat','SIR_SAR_FR'); %Path to L1a data
...
DDAcf = DDA_ConfigFile('CS','SAR',CS1a.GEO.Baseline_ID);
...
[~,CS1b_TMP] = Cryo_L1b_struct('SIR_SAR_L1.DBL',size(CS1a.FBR.data,4),DDAcf);
...
[~,CS1b]           = Cryo_L1b_struct('SIR_SAR_L1.DBL',size(CS1b_TMP.GEO.LAT,2),DDAcf);
...
CS1b.GEO.Beam      = CS1b_TMP.GEO.Beam;
clear('CS1b_TMP')
...
BA = permute(BA,[3,1,2]);
[CS1b.SAR.('BeamAngle'),CS1b.SAR.('Hmask')] = deal(cell(numel(find(~isnan(CS1b.GEO.LAT))),1));
for i=find(~isnan(CS1b.GEO.LAT))'
    CS1b.SAR.BeamAngle{i} = BA(CS1b.GEO.IDXstack{i});
    CS1b.SAR.Hmask{i}     = CS1a.FBR.Hmask(:,CS1b.GEO.IDXstack{i});
end
