...
elseif strcmp(dataset,'S6test_FF')
    ...
    FFSAR_processing_settings.output_pseudo_delay_doppler_processing = true;
    %FFSAR_processing_settings.output_pseudo_delay_doppler_processing = false;
    FFSAR_processing_settings.num_coherent_bursts = 1;
    %FFSAR_processing_settings.integration_time = DDAcf.BRI*180.01; % corresponding to 180 bursts
    %FFSAR_processing_settings.along_track_oversampling = 1/(0.25*585); % corresponding to ~80 Hz
    FFSAR_processing_settings.along_track_oversampling = 1; % corresponding to ~80 Hz
    ...
    %FFSAR_default_processing_settings.n_cores_parfor = feature('numcores');
...
%save(['/home/fehelers/PhD_Delft/pseudoDDprocessing/' dataset '_with_zD.mat'],'ffproc','-v7.3' )
%%
load(['/home/fehelers/PhD_Delft/pseudoDDprocessing/' dataset '_with_zD.mat'],'ffproc')
CS1b = ffproc.CS1b;
%%
...
%%
figure; 
plot(sum(CS1b.SAR.data_pseudoDD_zeroDop,2)./max(sum(CS1b.SAR.data_pseudoDD_zeroDop,2))); hold on; 
plot(sum(CS1b.SAR.data_pseudoDD,2)./max(sum(CS1b.SAR.data_pseudoDD,2))); hold on
plot(sum(CS1b.SAR.data,2)./max(sum(CS1b.SAR.data,2))); hold on
%%
figure;
...
comp.avg_zeroDop = nanmean(CS1b.SAR.data_pseudoDD_zeroDop,2);
...
plot(comp.avg_zeroDop./max(comp.avg_zeroDop),'Color',cDD,'LineWidth',lw,'LineStyle','-'); hold on
plot(comp.avg_FF./max(comp.avg_FF),'Color',cFF,'LineWidth',lw,'LineStyle','-'); hold on


legend('multilooked UF-SAR','UF-SAR zero-Doppler beam (nadir)','FF-SAR')
%xlabel('range bin')
ylabel('power')
ylim([0,1.05])
...
ax2 = subplot(4,1,3)
plot(100*(comp.avg_pseudo_DD./max(comp.avg_pseudo_DD)-comp.avg_FF./max(comp.avg_FF))./(comp.avg_FF./max(comp.avg_FF)),'Color',[0.5,0.5,0.5],'LineStyle','--'); hold on
plot(100*(comp.avg_zeroDop./max(comp.avg_zeroDop)-comp.avg_FF./max(comp.avg_FF))./(comp.avg_FF./max(comp.avg_FF)),'Color',[0.5,0.5,0.5]); hold on
...
ax3 = subplot(4,1,4)
plot(100*(comp.avg_pseudo_DD./max(comp.avg_pseudo_DD) - comp.avg_FF./max(comp.avg_FF)),'Color',[0.5,0.5,0.5],'LineStyle','--'); hold on
plot(100*(comp.avg_zeroDop./max(comp.avg_zeroDop) - comp.avg_FF./max(comp.avg_FF)),'Color',[0.5,0.5,0.5]); hold on
...
%exportgraphics(gcf,['/home/fehelers/PhD_Delft/pseudoDDprocessing/' dataset 'w_zeroDoppler.pdf'],'Resolution',300)
