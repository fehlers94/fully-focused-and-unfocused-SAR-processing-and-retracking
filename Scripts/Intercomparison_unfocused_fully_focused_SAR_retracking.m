...
save(['/home/fehelers/PhD_Delft/pseudoDDprocessing/' dataset '_' mode '_retracking.mat'],'ffproc','-v7.3' )
%%
load(['/home/fehelers/PhD_Delft/pseudoDDprocessing/' dataset '_' mode '_retracking.mat'],'ffproc')
CS1b = ffproc.CS1b;
clear ffproc
%%
...
text(54.371,38.13,{['bias =(',num2str(1000*mean(dataFF-dataUF),1),'\pm',num2str(1.96*1000*std_error,2),') mm',', R = ',num2str(R(1,2),2)]})
ylabel('uncorrected SSH (m)')
h = legend('FF-SAR; Full SAMOSA zero-Doppler beam fit (2.3 s)','UF-SAR; Full SAMOSA fit (2.3 s)')
...
R = corrcoef(dataFF,dataUF)
std_error = std(dataFF-dataUF)/sqrt(numel(dataFF)/1)
text(54.371,2.9,{['bias =(',num2str(100*mean(dataFF-dataUF),2),'\pm',num2str(1.96*100*std_error,2),') cm',', R = ',num2str(R(1,2),2)]})
ylabel('SWH (m)')
...
% %plot(L2.LAT_20(maskL2),L2.SIG0_20(maskL2))
% grid on
% R = corrcoef(dataFF,dataUF)
...
ax1.XLim = [54.3,54.5]
ax1.YLim = [37.5,38.2]
exportgraphics(gcf,['/home/fehelers/PhD_Delft/pseudoDDprocessing/paper_S3_retracking_results.pdf'],'Resolution',300)
